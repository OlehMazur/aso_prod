...
try:
    csv_file_main_data= sys.argv[1]
    csv_file_SKU =  sys.argv[2]
    csv_file_Shop = sys.argv[3]
    period = sys.argv[4]
    ...
    producer = sys.argv[9]
    city = sys.argv[10]
    encoding =  sys.argv[11]
    delimiter = sys.argv[12]
    data_version = sys.argv[13]
    use_clustering = bool(int(sys.argv[14]))
...
# python BG_2018_v2.csv BG_V2_SKU.csv BG_V2_Shop.csv month 7 8  Пятёрочка Пиво BALTIKA Москва cp1251 ; 001 0
...
if use_clustering:
    ...
    for ind, el in enumerate(np.unique(y_kmeans )):
        ...
        ff2_df = ff2_df[ (ff2_df['month'] >= start) & (ff2_df['month'] <= end)] #!!!!!!!!!!!!
